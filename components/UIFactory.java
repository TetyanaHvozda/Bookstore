...
public class UIFactory {
    ...
    public static void tableCoverColumn(Grid<Book> grid) {
        grid.addComponentColumn( cover -> {
/*            Span bookTitle = new Span( book.getTitle() );
                    bookTitle.setClassName("book-title");
            Span bookDesciption = new Span( book.getDescription() );
                    bookDesciption.setClassName("book-description");
            Span bookData = new Span( bookTitle, bookDesciption  );
                    bookData.setClassName("book-data");*/

            Span pictureColumn = new Span(UIFactory.loadImage( cover.getTitle(), cover.getCover() ));
                    pictureColumn.setClassName("picture-column");
    ...
    public static void tableAuthorColumn(Grid<Book> grid) {
        grid.addColumn( book -> book.getAuthors().stream().map(Author::getFullName).collect(Collectors.joining(", ")))
                .setHeader( "Author(s)" )
                .setTextAlign( ColumnTextAlign.CENTER )
                .setAutoWidth(true);
    }
    ...
    public static Image loadImage(String name, String imageUrl ) {
        ...
        loadImageSrcBook( name, imageUrl, image );
        return image;
    }

    public static void loadImageSrcBook(String name, String cover, Image image) {
        if(cover == null || cover.isBlank()) {
            image.setSrc( Application.DEFAULT_BOOK_SRC );
            return;
        }
    ...
    public static Avatar loadAvatar(String name, String imageUrl ) {
        ...
        if(imageUrl == null || imageUrl.isBlank()) {
            avatar.setImage( Application.DEFAULT_AVATAR_SRC );
        } else {
    ...
    public static RouterLink createNavItem(Class<? extends Component> view, Component icon) {
        ...
        // Read value of annotation @NavTitle from View
